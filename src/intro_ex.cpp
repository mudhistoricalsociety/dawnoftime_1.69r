...
void do_askname(char_data *ch, char * argument)
{
	...
	if(IS_NULLSTR(argument)){
		...
		ch->println("");
		ch->wrapln("Use askname to request a player to introduce themself to you... they will "
			"automatically introduce themself to you if they haven't turned off autoanswer.");
		return;
	}
...
void do_forget(char_data *ch, char * argument)
{
	...
	if(IS_NULLSTR(argument)){
		...
		ch->println("- removes their name beside their short after they have introduced themself");
		return;
	}
...
char * PERS( char_data *ch, char_data *looker)
{
	...
	strcpy(short_descr, mxp_create_tag(looker, mxp_tag_for_mob(looker, ch), ch->short_descr));

	char cname[MSL];
	strcpy(cname, mxp_create_tag(looker, mxp_tag_for_mob(looker, ch), capitalize(TRUE_CH(ch)->name)));
	char name[MSL];
	strcpy(name, mxp_create_tag(looker, mxp_tag_for_mob(looker, ch), TRUE_CH(ch)->name));
