...
void do_resetlu( char_data *ch, char *argument )
{
    ...
    else if ( fSpells )
    {
        ...
        ch->println( "Reseting lastused on all spells on char." );
        for ( sn = lower; sn <= upper; sn++ )
        {
            if ( skill_table[sn].name != NULL )
            victim->pcdata->last_used[sn] = 0;
...
void do_sset( char_data *ch, char *argument )
{
    ...
    else if ( fSpells )
    {
		...
		if(victim->pcdata->learned[sn] == value){
			ch->printf("No change in skill on %s.\r\n", victim->name);
		}else{
			ch->printf("Skill on %s changed from %d to %d.\r\n", 
				victim->name, victim->pcdata->learned[sn], value);
	        victim->pcdata->learned[sn] = value;
		}
    }
...
void do_mset( char_data *ch, char *argument )
{
    ...
    if ( !str_prefix( arg2, "pkool" ) )
    {
        ...
        if ( value > 15000 )
        {
            ch->println( "That is getting a bit high dont you think?" );
            return;
        }
    ...
    if ( !str_prefix( arg2, "pknorecall" ) )
    {
        ...
        if ( value > 50 )
        {
            ch->println( "That is getting a bit high dont you think? (try 50)" );
            return;
        }
	...
	if ( !str_prefix( arg2, "damroll" ))
	{
		...
		ch->println( "DO NOT FORGET TO PURGE THIS MOB IF YOU DONT INTEND FOR IT TO BE LIKE THIS PERMANENTLY!!!" );
...
void do_sockets( char_data *ch, char *argument )
{
    ...
    for ( c = connection_list; c; c = c->next )
    {
		...
		if(!IS_NULLSTR(arg)){
			...
			// if host name matches we dont do any other matching
...
void do_forcetick( char_data *ch, char *)
{
...
void do_pinfo( char_data *ch, char *argument )
{
    ...
    for ( wch = player_list; wch; wch = wch->next_player )
    {
		...
		// Format it up
		sprintf( buf, "%s[%s%1s%1s%3d %+2d/%+2d %-6.6s %-3.3s %3d %3d%3d %2d%s] %s%s%s%s-`x%s %s%s`x\r\n",
			bracket_col,
...
void do_rename (char_data* ch, char* argument)
{
	...
	{ // dont allow duplicates with names in creation/level 1 unsaved player
		connection_data *dc;
...
void do_rinfo( char_data *ch, char *)
{
    ...
    output= new_buf();
	sprintf( buf,"`?%s`x", makef_titlebar("RINFO")); add_buf(output,buf);
	...
	sprintf( buf,"`?%s`x", makef_titlebar("`xSee also the raceinfo command, `Y*`x=not creation selectable")); add_buf(output,buf);
...
void do_rwhere(char_data *ch, char *argument)
{
	...
	sprintf( buf,"`?%s`x", makef_titlebar("RESTRUNG OBJECTS WHERE"));
	add_buf(buffer,buf);
...
void do_freevnum(char_data *ch, char *argument)
{
	...
	if ( IS_NULLSTR( arg )){
		ch->printf("Syntax:  freevnum <obj/mob/room>\r\n");
		return;
    }
...
void do_iwhere( char_data *ch, char *)
{
	...
	sprintf( buf, "`^%s`x", makef_titlebar("IMMORTAL WHERE`^"));
	add_buf(buffer,buf);

	// turn off everyones DYN_TEMP_FLAG 
	...
	for ( vch = char_list; vch; vch = vch->next)
	{
		// dont directly list morts or those already list
	...
	sprintf( buf, "`^%s`x", makef_titlebar("%d TOTAL`^", count));
	add_buf(buffer,buf);
...
void do_pracsystester( char_data *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' )
	{
        ch->printf("Set whom as a prac/improve system tester?\r\n");			
		ch->printf("Dont put this on players unless you know what you are doing.\r\n");
        return;
    }
...
void do_immget(char_data *ch,char *argument)
{
    ...
    if( obj->wear_loc != WEAR_NONE ){
	    ...
	    victim->printlnf("%s has just unequiped %s from you, into your inventory.",ch->name,obj->short_descr);		
		return;
	}
    obj_from_char( obj );  
    obj_to_char( obj, ch );
