...
/**************************************************************************/
// prototypes
game_settings_type *game_settings;
...
const struct flag_type game_settings_flags2[] =
{
	...
	{ "dont_save_mobprog_authors",	GAMESET2_DONT_SAVE_MOBPROG_AUTHORS,	false},
...
const struct flag_type area_import_flags[] =
{
	...
	{ "discard_unfound_mobprogs",	AREAIMPORTFLAG_DISCARD_UNFOUND_MOBPROGS, true},	
...
bool gameedit_mintotal( char_data *ch, char *argument )
{
    if ( IS_NULLSTR(argument)) {
		...
		ch->wrapln( "This total value is approximately calculated basically by "
			...
			"adding an addtional 10 to the total.");
		return false;
	}
...
bool gameedit_no_resolve_ip(char_data *ch, char *argument)
{
    if ( IS_NULLSTR(argument)){
		...
		ch->println("Syntax: `=Cno_resolve_ip -`x to clear the dont resolve ip.");
...
void load_continents()
{
	...
	log_string("Continents file not found, creating default set of contients.");
	create_continents();
	save_continents();
}
