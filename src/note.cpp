...
/**************************************************************************/
bool new_is_note_to( char_data *ch, NOTE_DATA *pnote, bool reading_all)
{
	...
	{
		if ( is_exact_name( "bard", pnote->to_list ))
			return true;
	}

	if (IS_IMMORTAL(ch)){
		if ( is_exact_name( "council", pnote->to_list )){
			return true;
		}
		...
		// admin can read all those notes
		{
			...
			{
				if(is_exact_name( council_flags[index].name, pnote->to_list )){
					if (IS_ADMIN(ch)
	...
	if (IS_NOBLE(ch))
	{
		if ( is_name( "noble", pnote->to_list ) && 
			!is_exact_name( "noblepkill", pnote->to_list) )
			return true;

		if ((!reading_all || !IS_SET(ch->notenet, NOTE_TO_PKILL))
			&& is_exact_name( "pkill", pnote->to_list ))
			return true;

		if ((!reading_all || !IS_SET(ch->notenet, NOTE_NOBLEPKILL))
			&& is_name( "noblepkill", pnote->to_list ))
			return true;
    ...
    if (GAMESETTING2(GAMESET2_AUTONOTE_RENAMES_TO_ADMIN)
		&& IS_ADMIN(ch) 		
		&& is_exact_name( "adminrename", pnote->to_list )){	
    ...
    if ( IS_IMMORTAL(ch))
	{
		if (is_name( "imm", pnote->to_list ) ){
			if (is_exact_name( "immpkilldetails", pnote->to_list ) 
				|| is_exact_name( "immpkill", pnote->to_list )){
	...
	if (( ch->level==LEVEL_HERO 
				|| IS_ADMIN(ch)) && is_name( "hero", pnote->to_list )  )
			return true;

	if ( (IS_IMMORTAL(ch) || IS_QUESTER(ch))
		&& ( is_exact_name( "quester", pnote->to_list)))
		return true;

	if ( (IS_IMMORTAL(ch) || !IS_QUESTER(ch))
		&& ( is_exact_name( "nonquester", pnote->to_list)))
		return true;

	if ( HAS_SECURITY(ch,1) 
		&& ( is_name( "build", pnote->to_list)))
		return true;

	if ( HAS_SECURITY(ch, 5) && is_name( "olc", pnote->to_list)) 
		return true;

    if ( IS_ADMIN(ch) && is_name( "admin", pnote->to_list ) )
		return true;

    if ( TRUE_CH(ch)->level>=MAX_LEVEL && is_name( "imp", pnote->to_list ) )
		return true;

    if ( (IS_NEWBIE_SUPPORT(ch)||IS_IMMORTAL(ch)) && is_name( "nsupport", pnote->to_list ) )
		return true;

    if ( (IS_RP_SUPPORT(ch)||IS_IMMORTAL(ch)) && is_name( "rpsupport", pnote->to_list ) )
		return true;
	...
	if(GAMESETTING5(GAMESET5_NOTES_TO_RACE_NAMES_SUPPORTED)){
		...
		if(IS_IMMORTAL(ch)){
			...
			{
				if(is_exact_name(race_table[race]->name, pnote->to_list)){
					return true;
				}
			}
		}
	...
	if(GAMESETTING5(GAMESET5_NOTES_TO_CLASS_NAMES_SUPPORTED)){
		...
		if(IS_IMMORTAL(ch)){
			...
			{	
				if(is_exact_name(class_table[classIndex].name, pnote->to_list)){
					return true;
				}
			}
		}
    ...
    if (ch->clan && is_exact_name(ch->clan->notename(),pnote->to_list)){
		return true;
	}

	if ( ch->clan || IS_IMMORTAL(ch)) {
		if ((!reading_all || !IS_SET(ch->notenet, NOTE_TO_ALLCLAN))
			&& is_exact_name( "allclan", pnote->to_list ))
			return true;
	}
	...
	if (IS_IMMORTAL(ch) && (!reading_all || !IS_SET(ch->notenet, NOTE_CLANNOTES)) )
	{
		...
		{
            if (is_exact_name(pClan->notename(), pnote->to_list) )
			{
				return true;
			}
		}
...
{	
	return new_is_note_to( ch, pnote, false);
}
...
{	
	logf("Loading notes...");	
    load_thread(NOTE_FILE,&note_list, NOTE_NOTE, 14*24*60*60);		// 2 WEEKS
    load_thread(IDEA_FILE,&idea_list, NOTE_IDEA, 28*24*60*60);		// 1 MONTH
    load_thread(PENALTY_FILE,&penalty_list, NOTE_PENALTY, 0);		// PERM
    load_thread(NEWS_FILE,&news_list, NOTE_NEWS, 0);				// PERM
    load_thread(CHANGES_FILE,&changes_list,NOTE_CHANGES, 0);		// PERM
    load_thread(ANOTE_FILE,&anote_list,NOTE_ANOTE, 2*28*24*60*60);	// 2 MONTHS
    load_thread(INOTE_FILE,&inote_list,NOTE_INOTE, 2*28*24*60*60);	// 2 MONTHS
    load_thread(MISC_FILE,&misc_list,NOTE_MISC, 14*24*60*60);		// 2 WEEKS
    load_thread(SNOTE_FILE,&snote_list,NOTE_SNOTE, 14*24*60*60);	// 2 WEEKS
    load_thread(PKNOTE_FILE,&pknote_list,NOTE_PKNOTE, 28*24*60*60); // 1 MONTH
	notes_loaded=true;	
}
...
void load_thread(char *name, NOTE_DATA **list, int type, time_t free_time)
{
    ...
    exit_error( 1 , "load_notes", "bad key word");
    return;
}
...
//		if ( str_cmp( ch->name, pnote->sender ) && to_new[0] != '\0' )
		if ( str_cmp( ch->name, pnote->real_sender ) && to_new[0] != '\0' ) {
...
bool new_hide_note(char_data *ch, NOTE_DATA *pnote, bool reading_all){
	...
	if (!new_is_note_to(ch,pnote, reading_all))
		return true;
//	if (!is_note_to(ch,pnote))
//			return true;
...
char *note_format_noteentry_for_char(int vnum, NOTE_DATA *pnote, char_data *ch)
{
	...
	sprintf( buf, "`x[%s%s] %6s %-14s`W %-12s`x %s`x\r\n",
		mxp_vnum,
        alread_read_note? " " : "N", dateword,
        pnote->to_list, pnote->sender, pnote->subject );
    ...
    if ( !str_prefix( arg, "forward" ) )
    {
        ...
        for ( pnote = *list; pnote != NULL; pnote = pnote->next )
        {
            if ( is_note_to( ch, pnote ) && ( vnum++ == anum) )
            {
				...
				sprintf(newtext, "`G%s `Gwrote to `G%s "
					"`Gon %s\r\n-+== Forwarded %s follows ==+-`x\r\n",
						pnote->sender, pnote->to_list, pnote->date, list_name);
       ...
       // int ncount = count_notes(ch, *list);
        ...
        for ( pnote = *list; pnote != NULL; pnote = pnote->next )
        {
            if ( is_note_to( ch, pnote ) )
            {
                ...
                // search for note
				if (linenum<150
					&& ((!fWord&&(!str_infix( argument,pnote->to_list)
						...
						||
						(fWord&&(is_exact_name( argument,pnote->to_list)
...
////////////////////////////////////////////////////////////////
	if ( !str_prefix( arg, "show" ))
	{
		...
		ch->println( "=== NOTE SHOW, read `=Chelp note-show`x for other options ===" );
		ch->printlnf( "`x%s`x: %s`x\r\n`xTo: %s`x",
			ch->pnote->sender,
			ch->pnote->subject,
			ch->pnote->to_list );
