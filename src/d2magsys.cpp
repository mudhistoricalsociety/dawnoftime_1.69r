...
/****************************************************************************/
// Kal - July 99
void check_realm_improve(char_data * ch, int sn, bool success)
{
	...
	for(i=0; i < MAX_REALMS; i++){
		...
		case 3:
			...
			break;
		default:
			bugf("check_realm_improve(): Unknown i index %d!!!", i);
			return;
		}

		// loop thru each type of magic system checking for improvements
		if(bits){
			sprintf(buf, flag_string( table, -1));
			argument=buf;
			for (; ;)
			{
				...
				if(IS_SET(bits, flag_value(table,word))){
					...
					if(componentsn>-1){
						...
						check_improve(ch,componentsn,success,5);
					}
				}
			}
		}
...
bool check_learn_spell(char_data * ch, int sn, int one_out_of)
{
    ...
    chance=UMAX(get_avg(ch, sn)-10, 1);
	 ...
	 sprintf( wiznet_buf, "BETA_GET_AVG: %s '%s' (av= %d%%).",
	    ch->name,
		skill_table[sn].name,
		chance);
...
void on_level_learn(char_data * ch)
{
    ...
    for(i=FIRST_SPELL; i<=LAST_SPELL; i++)
    {
        ...
        // levels 1 thru 12 get a higher chance of learning a spell
        if (skill_table[i].skill_level[ch->clss]<1
            && skill_table[i].skill_level[ch->clss]>12)
            lchance=3;
        else
        {
            lchance = (skill_table[i].skill_level[ch->clss]+2)/3;
        }

        if(check_learn_spell(ch,i,lchance))
        {
            ch->pcdata->learned[i]=1;
            ch->printlnf("You have mastered the spell `Y%s`x.", skill_table[i].name);
            check_realm_improve(ch,i,true);  
        }
    }  
    return;
}
									...
									char have_colour, char havenot_colour)
{
	...
	char buf[MIL];
	result[0]='\0';
	result[1]='\0'; // incase we get an empty text
	...
	while(!IS_NULLSTR(text)){
		...
		strcat(result, buf);
		if(true){ // add on the players percentages
			if(sn>=0 && ch->pcdata && ch->pcdata->learned[sn]>0){
...
void do_spinfo(char_data *ch, char * argument)
{
	...
	for(j=FIRST_SPELL; j<=LAST_SPELL; j++)
	{
		...
		{
			ch->printf(get_spinfo_data(ch, j));
		}
	}
...
int spell_get_target_index(int sn)
{
	...
	switch(skill_table[sn].target){
		...
		case TAR_CHAR_DEFENSIVE:	index=2; break; // Defensive or Self-Only Spells (2)
		case TAR_CHAR_SELF:			index=2; break; 
		case TAR_OBJ_INV:			index=3; break; // Object Spells (3)
...
void do_spells(char_data *ch, char * argument)
{
    ...
    if (IS_NPC(victim))
	{
		ch->println( "Mobs dont have spells as such." );
		return;
	}
	...
	{
		sprintf(buf,"`^%s`x", makef_titlebar("SPELLS"));
	}
    add_buf( output, buf );
	...
	else
	{
		...
		for(t=0; t<MAX_SPELL_TARGET_NAMES; t++){
			add_buf( output, FORMATF("`^%s`x", makef_titlebar(spell_target_names[t])));
		...
		add_buf( output, FORMATF("`^%s`x", makef_titlebar("%d SPELL%s TOTAL", count, count==1?"":"S")));
		ch->sendpage(buf_string(output));
	}
...
void do_study(char_data * ch, char * argument)
{
	...
	for(i=1; i<=4; i++) // loop thru each spell
	{
		...
		if(check_learn_spell(ch,scroll->value[i],3))
		{
			...
			check_realm_improve(ch,scroll->value[i], true );
		}
		else
		{
			ch->printlnf( "You have failed to learn %s.",
				skill_table[scroll->value[i]].name);
			check_realm_improve(ch,scroll->value[i], false );
		}
	}
...
void do_teach(char_data *ch, char * argument)
{
	...
	{
		ch->println( "After you started the lesson, you realized student isn't familar with all " );
		...
		victim->wrapln( "Your teacher realized once the lesson started, that you did not know "
			...
			"lesson would have been too unpredicatable." );
		
//      check_realm_improve(ch,sn,false); // taken out due to cheaters
...
//      check_realm_improve(ch,sn,false); // taken out due to cheaters
	...
	if(check_learn_spell(victim,sn,2))
	{
		...
		check_realm_improve(ch,sn,true);
		check_realm_improve(ch,sn,true);
		check_realm_improve(ch,sn,true);
		check_realm_improve(victim,sn,true);
		check_realm_improve(victim,sn,true);
		check_realm_improve(victim,sn,true);
	}else{ 
		...
		// you dont improve if they can't learn the spell
		if(can_learn_spell(victim, sn))
		{
			check_realm_improve(ch,sn,false);
			check_realm_improve(ch,sn,false);
			check_realm_improve(victim,sn,false);
			check_realm_improve(victim,sn,false);
			check_realm_improve(victim,sn,false);
		}else{
...
/****************************************************************************
 * do_updatemagic - recalculates a players magic                            *
 * Written: Kalahn - Nov 97                                                 *
 ****************************************************************************/
void do_updatemagic(char_data *ch, char * argument)
{
    BUFFER *output;
    char_data *victim;
    char arg[MIL];
    char buf[MSL];
    int pracs, sn, slost_count, prac_count, sgain_count, tlevel;

	argument = one_argument( argument, arg );

	if(arg[0] == '\0' )
	{
        ch->println( "Syntax: updatemagic <player>" );
        return;
	}
    ...
    /* setup a buffer for info to be displayed and initialise our counters */
    output = new_buf();
    prac_count=0;
    slost_count=0;
    sgain_count=0;

    tlevel = victim->level + 5 - (victim->level/10);
    if (tlevel<victim->level) tlevel = victim->level;

    sprintf(buf, "Updating magic on %s [%d] (%s) <%d>.",
        victim->name, victim->level, class_table[victim->clss].name,
        tlevel);
    log_string(buf);
    append_datetimestring_to_file(UPDATE_MAGIC_FILE, buf);

    // check if person has already had update
    if (victim->level>=LEVEL_IMMORTAL)
    {
        log_string("Character is an immortal - aborting");
        victim->subversion=1;
        if (ch!=victim)
            ch->println( "UpgradeMagic isn't relevant to immortals." );
        free_buf(output);
        return;
    }
    ...
    if (victim->level<2)
    {
        ...
        victim->subversion=2;
        if (ch!=victim)
            ch->println( "UpgradeMagic isn't relevant to level 1 players." );
        free_buf(output);
        return;
    }

    if ((victim->version<9) || (victim->subversion<1 && victim->version==9))
    {
        log_string("This character has already had a magic update!");
		ch->printlnf( "%s has already had a magic update! - doing update regardless.",
			victim->name);
    }
    else
    {
        add_buf( output,
"\r\n`R             --=== M A G I C   S Y S T E M   U P D A T E ===--`x\r\n");

        sprintf(buf,
"We have just finished revamping the magic system and your player file is "
"being automatically upgraded to be inline with the new system.  Because of "
"this you may lose or gain some spells, unfortunately some people might lose "
"a lot of spells, and I apologize for that in advance, but in the long run it "
"is for the overall benefit of the game and game balance.");
        add_buf( output, format_string( str_dup(buf)));
        add_buf( output, "\r\n");

        sprintf(buf,
"The new system is designed in a way that will make teaching spells a lot "
"more effective, and gaining spells by leveling harder... it has also been "
"designed so players get the spells in the level range 1 thru 90, with the "
"majority between 10 and 70, instead of between 1-30... with only a few "
"spells being gained after level 30.");
        add_buf( output, format_string( str_dup(buf)));
        add_buf( output, "\r\n");

        sprintf(buf,
"Uptil now the system was malfunctioning, and that the spells you lose (if "
"you lose any) you should have never had in the first place.  I know that "
"sounds harsh... but this upgrade is a lot better than a player wipe to "
"balance things out you must agree.");
        add_buf( output, format_string( str_dup(buf)));
        add_buf( output, "\r\n");

        sprintf(buf,
"Everything done by this update is logged, so if you lose a spell that you "
"had practised, the update automatically rebates more than your pracs, at "
"what it would take you to reprac it (based off your current stats)."
"If you lose a spell that is above 80%%, when you gain the spell... you can "
"opt to have it set back to the value over 80%%, and we will just deduct the "
"pracs you got for the rebate - ie you wont lose those that extra percentage "
"that you can't train.");
        add_buf( output, format_string( str_dup(buf)));
        add_buf( output, "\r\n");

        add_buf( output, "\r\n`BStarting update now.`x\r\n");
    }

    for(sn=FIRST_SPELL; sn<=LAST_SPELL; sn++)
    {
        if (victim->pcdata->learned[sn]
            && (calc_spell_percent(victim->clss, tlevel, sn)< 3500) )
        {
            if ( !(check_learn_spell(victim,sn,1)
                 || check_learn_spell(victim,sn,1)))
            { // lose the spell
                // calc prac refund
                if (skill_table[sn].rating[victim->clss]>0)
                {
                    pracs= ( (victim->pcdata->learned[sn]/
                            (victim->modifiers[STAT_ME]+victim->modifiers[STAT_RE]+30))
                              * skill_table[sn].rating[victim->clss] );
                    if (victim->pcdata->learned[sn]>1) pracs++;
                }
                else
                    pracs=0;

                sprintf(buf,"%s lost %-20s was at %d%% (got %d pracs) [%2d,%2d,%2d]",
                    victim->name,
                    skill_table[sn].name,
                    victim->pcdata->learned[sn],
                    pracs,
                    skill_table[sn].skill_level[victim->clss],
                    skill_table[sn].rating[victim->clss],
                    skill_table[sn].low_percent_level[victim->clss]
                    );
                log_string(buf);
                append_timestring_to_file(UPDATE_MAGIC_FILE, buf);

                sprintf(buf,"You have lost the spell `Y%s`x from magic revamp!\r\n",
                    skill_table[sn].name);
                add_buf( output, format_string( str_dup(buf)));

                sprintf(buf,"You had that at %d%% and received a %d prac rebate.\r\n",
                    victim->pcdata->learned[sn], pracs);
                add_buf( output, format_string( str_dup(buf)));
                    
                victim->pcdata->learned[sn]=0;
                victim->practice+=pracs;
                prac_count+=pracs; // record the pracs they get
                slost_count++;
            }
        }
        else // a chance of learning it
        {
            if(check_learn_spell(victim,sn,1))
            {
                victim->pcdata->learned[sn]=1;
                sprintf(buf,"%s gained '%s'.",
                    victim->name,
                    skill_table[sn].name);
                log_string(buf);
                append_timestring_to_file(UPDATE_MAGIC_FILE, buf);

                sprintf(buf,"You have gained the spell `Y%s`x from magic revamp!\r\n",
                    skill_table[sn].name);
                add_buf( output, format_string( str_dup(buf)));
    ...
    if (victim->subversion==0)
    {
        add_buf( output, "`BUpdate completed.`x\r\n");
    
        sprintf(buf, "(You lost %d spells, gained %d spells, and got %d pracs refunded.)",
            slost_count, sgain_count, prac_count);
        add_buf( output, format_string( str_dup(buf)));
        add_buf( output, "\r\n");
    }

    victim->subversion=2;
    sprintf(buf, "Update magic on %s completed. (pgain=%d, sloss=%d, sgain=%d)",
        victim->name, prac_count, slost_count, sgain_count);
    log_string(buf);
    append_timestring_to_file(UPDATE_MAGIC_FILE, buf);

    /* tell the player about the spells the lost and the update */
    if (slost_count || sgain_count)
        victim->sendpage(buf_string(output));
