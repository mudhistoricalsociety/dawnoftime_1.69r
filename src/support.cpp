...
void do_checkhelp( char_data *ch, char *argument)
{
	...
	sprintf( buf,"`?%s`x", makef_titlebar("NOHELP INFO"));
	add_buf(output,buf);
...
void do_checkooc( char_data *ch, char *argument)
{
	...
	sprintf( buf,"`?%s`x", makef_titlebar("OOC LOGFILE"));
	add_buf(output,buf);
...
void do_checkmoblog( char_data *ch, char *argument)
{
	...
	sprintf( buf,"`?%s`x", makef_titlebar("MOBLOG LOGFILE"));
	add_buf(output,buf);
...
void do_checknsupport( char_data *ch, char *argument)
{
    ...
    sprintf( buf,"`?%s`x", makef_titlebar("NSUPPORT LOGFILE"));
	add_buf(output,buf);
...
void do_checkntalk( char_data *ch, char *argument)
{
	...
	sprintf( buf,"`?%s`x", makef_titlebar("NTALK LOGFILE"));
	add_buf(output,buf);
...
void do_checktypos( char_data *ch, char *argument)
{
	...
	sprintf( buf,"`?%s`x", makef_titlebar("TYPO INFO"));
	add_buf(output,buf);
...
void do_list_letgains( char_data *ch, char *argument )
{
	...
	if(!IS_NULLSTR(argument))
	{
		...
		if(node){
			...
			ch->printlnf("`=RCreated: `=r%-24.24s", (char*)ctime((time_t*)&node->id));
...
void do_offlineletgain( char_data *ch, char *argument )
{
	...
	if (!node){
		ch->printlnf("There is noone in the letgain database with a name of '%s'.",
			name);
		return;
	}
...
void do_checkmultilog( char_data *ch, char *argument)
{
	...
	sprintf( buf,"`?%s`x", makef_titlebar("MULTILOG RECORDS"));
	add_buf(output,buf);
...
void do_checkcode( char_data *ch, char *argument)
{
	...
	sprintf( buf,"`?%s`x", makef_titlebar("CODING LOG"));
	add_buf(output,buf);
	...
	if (IS_NULLSTR(arg1))
	{
		sprintf(buf, "tail " CODELOG_FILE " -n 20");
...
void do_checknewbie( char_data *ch, char *argument)
{
	...
	sprintf( buf,"`?%s`x", makef_titlebar("NEWBIETALK LOGFILE"));
	add_buf(output,buf);
...
void do_checkclanbank( char_data *ch, char *argument)
{
	...
	sprintf( buf,"`?%s`x", makef_titlebar("CLANBANKING INFO"));
	add_buf(output,buf);
