...
GIO_SHINTH(m_RecallRoom,		"Recall		  " )
GIO_LONGH( m_BankRoom,			"BankRoom     " )
...
void do_clanedit( char_data *ch, char *argument )
{
	...
	if ( !str_cmp( arg, "create" ))
	{
		...
		if(clan_lookup(arg)){
			ch->printlnf("There already exists a clan with an exact or simular name to '%s', you must use another name sorry.", arg);
			return;
		}
