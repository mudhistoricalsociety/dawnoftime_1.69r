...
void load_arearom( FILE *fp )
{
	...
	pArea->olc_flags    = OLCAREA_LOADING;
	...
	if ( area_last  )
    {
		...
		REMOVE_BIT(area_last->olc_flags, OLCAREA_LOADING);
    }
		...
		case 'C':
			...
			if ( !str_cmp( word, "Continent" ) ){
				...
				if ( !pArea->continent){										
					...
					pArea->continent->name=str_dup(ltrim_string(rtrim_string(t)));
...
void load_mobprogs( FILE *fp )
{
    ...
    if ( area_last == NULL )
    {
		...
		exit_error( 1 , "load_mobprogs", "no #AREA seen yet");
    }
    ...
    for ( ; ; )
    {
		...
		if ( letter != '#' )
		{
			bugf("Load_mobprogs: # not found, letter='%c'.", letter);
			exit_error( 1 , "load_mobprogs", "# not found");
		}
		...
		if ( get_mprog_index( vnum ) != NULL )
		{
			...
			exit_error( 1 , "load_mobprogs", "duplicate vnum");
		}
		fBootDb = true;
		...
		// with area format version 4 and above mobprogs have a name
		if(area_last->version>3)
		{
			...
			// - dont bother freeing memory, since will be fixed 
...
void fix_mobprogs( void )
{
		...
		pMobIndex   = pMobIndex->next )
		{
			...
			{				
				if ( ( prog = get_mprog_index( list->temp_mpvnum ) ) != NULL ){
					...
					if(AREA_IMPORT_FLAG(AREAIMPORTFLAG_DISCARD_UNFOUND_MOBPROGS)){
						bugf( "Fix_mobprogs(): discarding code vnum %d not found on mob %d (%s) from %s.", 
							list->temp_mpvnum, pMobIndex->vnum, 
							pMobIndex->short_descr, pMobIndex->area->file_name);
						if(prev){
							prev->next=list->next;
						}else{
							pMobIndex->mprogs=list->next;
						}
					}else{
						bugf( "Fix_mobprogs(): code vnum %d not found on mob %d (%s) from %s.", 
							list->temp_mpvnum, pMobIndex->vnum, 
							pMobIndex->short_descr, pMobIndex->area->file_name);
						terminate=true;										
					}
	...
	if(terminate){
		bug("Due to unfound mobprogs, unable to boot mud.");
		bug("If you want to boot the mud without these progs\n"
			"and lose references to them you can add \n"
			"'discard_unfound_mobprogs' to gameset.txt area_import_flags");
		exit_error( 1 , "fix_mobprogs", "unable to boot with missing mobprogs");
	}
}
...
char *get_areaname(char_data *ch, AREA_DATA *pArea, bool longformat)
{
	...
	if(longformat){
		...
		if (IS_SET(pArea->area_flags, AREA_OLCONLY)){
			if(c_str_len(aname)>30){
				sprintf(areaname,"%s `g(OLC)`x", str_width(aname,30));
			}else{
				strcat(aname," `g(OLC)`x");				
				sprintf(areaname,"%s", str_width(aname,36));
			}
		}else{
			sprintf(areaname,"%s`x",str_width(aname,36));
		}
	}else{
	...
	if(longformat){
		sprintf( buf, "%s%s `^%s`x",
			areaname,
			(IS_NULLSTR(pArea->credits)?str_width(pArea->builders,20):str_width(pArea->credits, 20)),
			str_width(lrange, 12));
	}else{
		sprintf( buf, "%s %s`x ",
			areaname,
...
extern int mpqueued_count;
extern int mpqueued_total;
...
void do_memory( char_data *ch, char *)
{
	...
	ch->printlnf(" mpqueued_count=%d, mpqueued_total=%d",
		mpqueued_count, mpqueued_total);
    ...
    getcwd(cwdbuf, MSL);
#endif
	ch->printlnf(" Current working directory is %s.", cwdbuf);
