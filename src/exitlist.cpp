...
void room_pair (ROOM_INDEX_DATA* left, ROOM_INDEX_DATA* right, exit_status ex, char *buffer)
{
	...
	{
		char dcodes[MSL];
		sprintf(dcodes,	room_door_codes(left,right, col));
		...
		sprintf(dcodes,	room_door_codes(right,left, col));
...
void do_exitlist (char_data *ch, char * )
{
	...
	sprintf( buf3,"`S%s`x", makef_titlebar(buf2));
		add_buf(final,buf3);
				...
				if (pRoom)
				{
					if (pRoom->area == pArea)
					{
						sprintf(buf, "`x %-8.8s`S[`r%s(%s)`x %5d `#`R>>>>`^ %5d`S]`x\r\n", 
							...
							str_width(capitalize(pObj->short_descr),25), 
							str_width(pObj->name,22), 						
