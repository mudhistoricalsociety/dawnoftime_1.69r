...
void do_send( char_data *ch, char *argument )
{
	...
	{
        ch->println("SEND - sends the newbie document to a player.");
        ch->println("Syntax: send charname <email address>");
        return;
	}

    if ( ( victim = get_char_world( ch, arg1 ) ) == NULL )
    {
        ch->println("That character isn't online.");
        return;
    }

	if(IS_NPC(victim))
	{ 
        ch->println("*bonk self* You can't send the newbie doc to a NPC!.");
		return;
	}
    ...
    ch->printlnf("Newbie document sent to (%s) %s.", victim->name, arg2);
...
void wiznet_colourindex( int index, char_data *ch, char *argument )
{
	...
	ch->printbw(buf);
	ch->println("Note, you dont have to put only colour codes in the prefix.");	
}
/**************************************************************************/
...
void do_at( char_data *ch, char *argument )
{
    ...
    {
        ch->printlnf( "You are already in room %d... doing command without at.", 
			location->vnum);
...
void do_goto( char_data *ch, char *argument )
{
    ...
    if ( IS_SWITCHED(ch))
    {
        ...
        ch->println( "mob you are controlling type trans self <location>" );
        return;
    }
...
void do_charinfo( char_data *ch, char *argument )
{
	...
	ch->printf(  " `xName:`B %s`x   Created: %s", victim->name, ctime( (time_t *) & (victim->id)));
...
void do_trust( char_data *ch, char *argument )
{
	...
	if( victim->level== level ){
		ch->println("A trust of 0 is the same as a trust of their level... setting it to 0.");
		level=0;
	}
...
void do_restore( char_data *ch, char *argument )
{
	...
	if (IS_SILENT(ch)){
		ch->printlnf("Ok - restore done sliently on %s.", victim->name );
	}else{
...
void do_boon( char_data *ch, char *argument )
{
	...
	if (IS_SILENT(ch)){
		ch->printlnf("Ok - boon done sliently on %s.", victim->name );
	}else{
