...
/////////////////////////////////////////////////////////////////////
	...
	{
		ch->printlnf( "%s`#`s%s is tethered here.`&",
			buf, capitalize( PERS( victim, ch )));
        return;
	}
	...
	if (IS_RIDING(victim))
	{
		ch->printlnf( "%s`#`s%s is here riding `&%s.`B",
			buf,
...
void do_gprompt(char_data *ch, char *argument)
{
	...
	if( !str_cmp(argument,"help") )
	{
		...
		ch->println(
			...
			"`1");
		ch->println("Anything between %p and %P is 'eaten' when you dont have a pet in the room.");
		ch->println("Anything between %g and %G is 'eaten' when you dont have a group member in the room.");
    ...
    else
    {
        ...
        case POS_RESTING:  
            if ( ch->on != NULL )
            {
                ...
                {
                    sprintf( message, "resting in %s.", ch->on->short_descr );
                    strcat( buf, message );
                }
            }
            else
        ...
        case POS_SITTING:
            if ( ch->on != NULL )
            {
				...
				{
					sprintf( message, "siiting under %s", ch->on->short_descr );
					strcat( buf, message );
				}
                else
                {
...
void do_affects(char_data *ch, char * argument)
{
	...
	{
		if(v==ch){
			ch->println("`rYou are affected by the following spells:`x");
		}else{
			ch->printlnf( "`r%s is affected by the following spells:`x", PERS(v, ch));
		}
		for ( paf = v->affected; paf; paf = paf->next )
		{
			...
			{
				ch->printf( "Spell: %-19s", skill_table[paf->type].name );
			}			
		...
		if (i>-7) // can't see the moon really at this time of night
		{
			switch (time_info.hour)
			{
				case 20:
					...
					break;
				case 21:
					ch->println("The moon rises just above the the horizon.");
					break;
				case 22:
				...
				case 2:
					...
					break;
				case 3:
					ch->println("The moon is falling just below the the horizon.");
					break;
				case 4:
...
void do_equipment( char_data *ch, char * )
{
	...
	for ( iWear = 0; iWear < MAX_WEAR; iWear++ )
	{
		...
		if ( ( obj = get_eq_char( ch, iWear ) ) == NULL ){
			ch->println( "nothing.`=\xab");
			continue;
		}
		
		if ( can_see_obj( ch, obj ) )
		{
			ch->printlnf( "%s`=\xab", format_obj_to_char( obj, ch, true ));
...
void do_practice( char_data *ch, char *argument )
{
	...
	if(!IS_ICIMMORTAL(ch)){
		if(!can_see(mob, ch)){
			ch->printlnf("It looks as though %s would be able to help you practice,\r\n"
...
void do_quester(char_data *ch, char *argument)
{
	...
	if(!codehelp(ch, "do_quester_enabled", CODEHELP_ALL_BUT_PLAYERS )){
		...
		ch->println("Mobprogs that are programmed to work with only questers will recognise you now.");
	}
...
void do_losepet( char_data *ch, char *argument)
{
	...
	if(ch->fighting)
    {
        ch->println("You can't lose your pet while fighting.");
        return;
	}

	if (str_cmp("confirm", argument))
    {
        ch->println("`xType `=Closepet confirm`x to get rid of your pet.");
		return;
	}
	...
	ch->println("You have managed to lose your pet.");
