...
bool medit_show(char_data *ch, char *)
{
	...
	if(!IS_SET(ch->dyn,DYN_SHOWFLAGS)){
		...
		if ( pMob->mprogs )
		{
			...
			ch->printlnf("\r\n`=rMOBPrograms for [%5d]:", pMob->vnum);
			ch->printlnf("Mobprogs on this mob %s ignore questers", 
				IS_SET(pMob->act,ACT_MPIGN_QUESTER)?"WILL":"will not");
			ch->printlnf("Mobprogs on this mob %s ignore non questers", 
...
//					mxp_create_send(ch,FORMATF("mpshow %d", list->prog->vnum),
//										FORMATF("%4d", list->prog->vnum)),
...
bool medit_addmprog(char_data *ch, char *argument)
{
	...
	{
        ch->printlnf("No such MOBProgram %d.", mpnum);
        return false;
	}

    if(pMob->area!= get_vnum_area( mpnum )){
		if(HAS_SECURITY(ch,9)){
			ch->println("`RWARNING: That mobprog number belongs to a different area!`x");
		}else{
			ch->println("`RThat mobprog number belongs to a different area!`x");
			ch->println("`ROnly security 9 can link in mobprogs from different areas.`x");
			return false;
		}
	};
	...
	{
		ch->printlnf("Valid mobprog trigger flags are: %s", 
...
// Kal - Nov 99
long calc_mprog_flags(MPROG_LIST *mprogs)
{
    MPROG_LIST *list;
	long result=0;
...
bool medit_delmprog(char_data *ch, char *argument)
{
    ...
    if ( !(list= pMob->mprogs) )
    {
        ch->printlnf("MEdit: There are no mobprogs on mob template %d.", pMob->vnum);
        return false;
    }
	...
	// recalc the trigger bits
	long triggerbits=calc_mprog_flags(pMob->mprogs);
	if(triggerbits!=pMob->mprog_flags){
		pMob->mprog_flags=triggerbits;
		
		ch->printf("Mob '%s' triggering disabled, because there are no "
			"more progs that use the trigger type.\r\n", 
			mprog_type_to_name(trigtype));
...
bool medit_posmprog(char_data *ch, char *argument)
{
    ...
    {
		ch->println("Syntax:  posmprog <#mprog> <postion flag to toggle>");
		return false;
    }
...
/**************************************************************************/
// by Kal - June 98
static MPROG_LIST * dup_mprog_list(MPROG_LIST * mplist)
{	
	MPROG_LIST * pMplist;
	...
	pMplist->next = dup_mprog_list(mplist->next);
...
bool medit_copy(char_data *ch, char *argument)
{
	...
	// NOTE: we aren't deallocating the existing mobprog lists and 
	//	shops... we should, but I can't be bothered writing the code 
	// to do so right now.

	// copy mobprogs list
	pMob->mprogs		= dup_mprog_list(pSrc->mprogs);
...
void do_mxpmodlist( char_data *ch, char *argument )
{
	...
	{
		ch->printf("The mxpmodlist command is an olc related command, "
...
void do_mobloglist( char_data *ch, char *argument)
{
	...
	if (!HAS_SECURITY(ch,1))
	{
		if(str_cmp(argument, "auto")){
			ch->printf("The mobloglist command is an olc related command, "
				"you dont have olc permissions.\r\n");
		}
		return;
	}
	...
	if(!str_cmp(argument, "auto")){
		add_buf( buf1, 
			"\r\n"
			"The moblog system is used to debug mob interaction when mobprogs are in use.\r\n"
...
bool medit_mdelete(char_data *ch, char *argument)
{
    ...
    if (IS_NULLSTR(argument))
    {
		...
		ch->println("* Noone else can currently be editing the mobile.");
        ch->wrapln("NOTE: It is strongly recommended that no mobprogs attempt to load the mob "
			"you are considering deleting... the easiest method to do this is 'textsearch mobprog <mobvnum>'.");
		return false;
    }
